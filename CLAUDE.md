...
Do not modify, move, or delete existing files outside this repository. Test files anywhere outside this repository are live data: they need the owner's go-ahead under the rule below, and when allowed must be clearly named `TESTING: ...` and cleaned up when done.
